...
public class OutHandler extends Thread {
	...
	public void run() {
		String msg = null;
		try {
			LocalGlobalConfig.debugInfo(id + "开始推流！");
			
			while (desstatus && (msg = br.readLine()) != null) {
				/**
				 * 解析消息，在消息解析器中解析消息来判断当前任务的运行状态
				 * |比如：有返回错误消息的可以直接判断出来
				 * |长时间没有消息也可以让它重启，可以通过重新实现消息解析器来实现。
				 */
				ohm.parse(id,msg);
				if(ohm.isbroken()) {
					//如果发生异常中断，立即进行保活
					...
					LocalGlobalConfig.debugInfo("检测到中断，提交重启任务给保活处理器");
				}
			}
		} catch (IOException e) {
			LocalGlobalConfig.debugInfo("发生内部异常错误，自动关闭[" + this.getId() + "]线程");
			destroy();
		} finally {
			if (this.isAlive()) {
				destroy();
